...
export function createCSRFToken(): string {
  ...
  if (storage) {
    // Store in storage with metadata
    storage.setItem('oauth_csrf_token', JSON.stringify(token));
  }
...
export function validateCSRFToken(providedToken: string): boolean {
  try {
    ...
    const stored = storage.getItem('oauth_csrf_token');
...
export function clearCSRFToken(): void {
  const storage = getEffectiveStorage();
  if (storage) {
    storage.removeItem('oauth_csrf_token');
  }
}
